/****************************************************
 maintenance.js — Department roster + task checklist
 - Matches gardener/night-shift pattern exactly
 - Merges Admin + Manager tasks for dept-common.js via __getDeptTasks
 - No floating task panel; uses dept-common tables
****************************************************/
(function () {
  /* ========== CONFIG / CONSTANTS ========== */
  const TYPE_LOWER = 'maintenance';      // lower-case key for matching dept
  const TYPE_LABEL = 'Maintenance';      // display label
  ...
  const toast = (msg) => {
    const t = qs('#maint-toast') || qs('#toast') || qs('#room-toast');
  ...
  const log = (...a)=>{ try{ console.log('[maintenance]',...a);}catch{} };
  ...
  function localMock() {
    // Example fallback staff for Maintenance
    return normalizeStaff([
      { name: 'Sanjay Patel',   role: 'Technician',      type: TYPE_LABEL, status: 'Active',   assigned: 1 },
      { name: 'Meera Iyer',     role: 'Electrician',     type: TYPE_LABEL, status: 'Active',   assigned: 1 },
      { name: 'Karthik Rao',    role: 'Plumber',         type: TYPE_LABEL, status: 'Active',   assigned: 0 },
      { name: 'Asmita Ghosh',   role: 'HVAC',            type: TYPE_LABEL, status: 'Active',   assigned: 0 },
      { name: 'R. Banerjee',    role: 'Support',         type: TYPE_LABEL, status: 'Inactive', assigned: 0 },
    ]);
  }
  ...
  async function fetchState() {
    try {
      ...
      // Match by type or role containing "maintenance"
  ...
  const modal       = qs('#maint-modal') || qs('#modal') || qs('#room-modal');
  const modalTitle  = qs('#maint-modalTitle') || qs('#modalTitle') || qs('#room-modalTitle');
  const mName       = qs('#maint-mName') || qs('#mName') || qs('#room-mName');
  const mType       = qs('#maint-mType') || qs('#mType') || qs('#room-mType');
  const mStatus     = qs('#maint-mStatus') || qs('#mStatus') || qs('#room-mStatus');
  const mAssigned   = qs('#maint-mAssigned') || qs('#mAssigned') || qs('#room-mAssigned');
  ...
  document.addEventListener('click', (e) => {
    ...
    if (btn.id === 'btnAdd' || btn.id === 'maint-openModal') return openAdd();
    if (btn.id === 'maint-closeModal' || btn.id === 'closeModal' || btn.id === 'room-closeModal') return closeModal();
    if (btn.id === 'maint-cancelModal' || btn.id === 'cancelModal' || btn.id === 'room-cancelModal') return closeModal();
    if (btn.id === 'maint-saveModal' || btn.id === 'saveModal' || btn.id === 'room-saveModal') return save();
