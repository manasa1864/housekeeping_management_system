/****************************************************
 room-cleaning.js — Department roster + task checklist
 - Preserves your original roster/modal/search logic
 - Adds Admin↔Staff task sync (localStorage + events)
 - Uses TaskWire if available; else safe fallback
****************************************************/

(function () {
  /* ========== CONFIG / CONSTANTS ========== */
  const TYPE_LOWER = 'room cleaning';        // relaxed matching
  const TYPE_LABEL = 'Room Cleaning';        // display & exact type in tasks
  ...
  const TaskBus = (function () {
    // prefer global TaskWire if you included task-sync.js
    if (window.TaskWire) {
      return {
    ...
    function _writeAll(tasks) {
      ...
      window.dispatchEvent(new Event(UPDATE_KEY)); // same-tab pulse
    }
  ...
  const toast = (msg) => {
    const t = qs('#room-toast') || qs('#toast');
  ...
  /* ========== STATE ========== */
  let staffList = [];
  let allTasks  = []; // keep tasks in memory to show live counts in table

  /* ========== STAFF NORMALIZATION (unchanged from your file) ========== */
  ...
  function localMock() {
    return normalizeStaff([
      { name: 'Mark Clean', role: 'Housekeeper', type: TYPE_LABEL, status: 'Active', assigned: 4 },
      { name: 'Lucy Sweep', role: 'Assistant',    type: TYPE_LABEL, status: 'Active', assigned: 1 }
    ]);
  }

  const log = (...args) => { try { console.log('[room]', ...args); } catch {} };

  /* ========== FETCH STAFF FROM API (kept) ========== */
  async function fetchState() {
    try {
      ...
      staffList = localMock().filter(s => ((s.type || s.role || '').toLowerCase().includes(TYPE_LOWER)));
      renderTable();
    }
  }
  ...
  function renderTable() {
    ...
    if (!tbody) {
      log('tbody not found in DOM (#tbody). Check room-cleaning.html table markup.');
      return;
    }
    ...
    // compute live pending-by-assignee from tasks
  ...
  /* ========== MODAL / CRUD (unchanged behavior) ========== */
  const modal       = qs('#room-modal') || qs('#modal');
  const modalTitle  = qs('#room-modalTitle') || qs('#modalTitle');
  const mName       = qs('#room-mName') || qs('#mName');
  const mType       = qs('#room-mType') || qs('#mType');
  const mStatus     = qs('#room-mStatus') || qs('#mStatus');
  const mAssigned   = qs('#room-mAssigned') || qs('#mAssigned');
  ...
  /* ========== TASK LIST (Room Cleaning only) ========== */
  function renderDeptTasks() {
    const host =
      document.getElementById('taskPanel') ||
      document.getElementById('taskList') ||
      document.getElementById('roomTasks') ||
      null;

    // If no host exists, create a lightweight floating panel
    let created = false;
    let container = host;
    if (!container) {
      created = true;
      container = document.createElement('div');
      container.id = 'roomTasksAuto';
    ...
    const visible = (allTasks || []).slice().reverse().filter(t =>
      String(t.type || '').trim().toLowerCase() === TYPE_LOWER
    );
        ...
        div.innerHTML = `
          <div style="max-width:70%">
            ...
            <div style="font-size:12px;opacity:.8">
              Assignee: ${escapeHtml(t.assignee || '-')} • Room: ${escapeHtml(t.room || '-')}
            </div>
  ...
  document.addEventListener('click', (e) => {
    ...
    // Modal controls
    if (btn.id === 'btnAdd') return openAdd();
    if (btn.id === 'room-closeModal' || btn.id === 'closeModal') return closeModal();
    if (btn.id === 'room-cancelModal' || btn.id === 'cancelModal') return closeModal();
    if (btn.id === 'room-saveModal' || btn.id === 'saveModal') return save();
    ...
    if (completeId) {
      ...
      if (ok) {
        ...
        // refresh tasks + table live counts
  ...
  // Search / filter re-render for table
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // set page meta for other utilities if they read it
    window.STAFF_PAGE = window.STAFF_PAGE || { type: TYPE_LABEL, staffName: '' };

    // initial loads
    allTasks = TaskBus.readAll();
    fetchState();         // loads roster
    renderDeptTasks();    // show current tasks for Room Cleaning
  });

})();
/****************************************************
 room-cleaning.js — sync admin + manager tasks
****************************************************/
(function () {
