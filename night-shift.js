/****************************************************
 night-shift.js — Department roster + task checklist
 - Matches gardener pattern exactly
 - Merges Admin + Manager tasks for dept-common.js via __getDeptTasks
 - No floating task panel; uses dept-common tables
****************************************************/
(function () {
  /* ========== CONFIG / CONSTANTS ========== */
  const TYPE_LOWER = 'night shift';      // used for matching dept (lowercase)
  const TYPE_LABEL = 'Night Shift';      // display label
  ...
  const toast = (msg) => {
    const t = qs('#night-toast') || qs('#toast') || qs('#room-toast');
  ...
  const log = (...a)=>{ try{ console.log('[night-shift]',...a);}catch{} };
  ...
  // Expose merged list for dept-common.js (so dept pages see Admin + Manager)
  ...
  function localMock() {
    // Example fallback staff for Night Shift
    return normalizeStaff([
      { name: 'Arun Menon',     role: 'Night Supervisor', type: TYPE_LABEL, status: 'Active',   assigned: 1 },
      { name: 'Ritika Shah',    role: 'Night Attendant',  type: TYPE_LABEL, status: 'Active',   assigned: 1 },
      { name: 'Deepak Singh',   role: 'Night Attendant',  type: TYPE_LABEL, status: 'Active',   assigned: 0 },
      { name: 'Jaya Pillai',    role: 'Night Porter',     type: TYPE_LABEL, status: 'Active',   assigned: 0 },
      { name: 'S. Fernandez',   role: 'Support',          type: TYPE_LABEL, status: 'Inactive', assigned: 0 },
    ]);
  }
  ...
  async function fetchState() {
    try {
      ...
      // Match by type or role containing "night shift"
  ...
  const modal       = qs('#night-modal') || qs('#modal') || qs('#room-modal');
  const modalTitle  = qs('#night-modalTitle') || qs('#modalTitle') || qs('#room-modalTitle');
  const mName       = qs('#night-mName') || qs('#mName') || qs('#room-mName');
  const mType       = qs('#night-mType') || qs('#mType') || qs('#room-mType');
  const mStatus     = qs('#night-mStatus') || qs('#mStatus') || qs('#room-mStatus');
  const mAssigned   = qs('#night-mAssigned') || qs('#mAssigned') || qs('#room-mAssigned');
  ...
  document.addEventListener('click', (e) => {
    ...
    if (btn.id === 'btnAdd' || btn.id === 'night-openModal') return openAdd();
    if (btn.id === 'night-closeModal' || btn.id === 'closeModal' || btn.id === 'room-closeModal') return closeModal();
    if (btn.id === 'night-cancelModal' || btn.id === 'cancelModal' || btn.id === 'room-cancelModal') return closeModal();
    if (btn.id === 'night-saveModal' || btn.id === 'saveModal' || btn.id === 'room-saveModal') return save();
